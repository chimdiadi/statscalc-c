...
* C++ source files:
  * `include/statscalc-c/statscalc-c.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/statscalc-c.cpp` is the main file that implements this library.
  * `app/statscalc-c_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/statscalc-c_t.cpp` contains the unit tests for the library.
